        ...
        try {
            ...
            for (const finding of findings) {
                const range = new vscode.Range(
                    finding.start.line - 1,
                    finding.start.col - 1,
                    finding.end.line - 1,
                    finding.end.col - 1
                );
                const diagnostic = new vscode.Diagnostic(range, finding.extra.message, 
                    finding.extra.severity === 'ERROR' ? vscode.DiagnosticSeverity.Error : vscode.DiagnosticSeverity.Warning);
...
export class SecurityFixer implements vscode.CodeActionProvider {
    ...
    private createHardcodedSecretFix(document: vscode.TextDocument, range: vscode.Range): vscode.CodeAction | undefined {
        ...
        if (equalSignIndex === -1) return undefined;
